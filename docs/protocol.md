...
| --- | --- |
| `webrtc_offer` | `sdp`, optional `sdpType`, optional `sessionId` |
| `webrtc_answer` | `sdp`, optional `sdpType`, echo `sessionId` when present |
| `webrtc_ice` | `candidate` object or candidate fields, echo `sessionId` when present |

`sessionId` is generated by the App for each WebRTC offer attempt. The PC
bridge echoes it in answer and ICE messages so the App can ignore stale
signaling packets from a previous retry.
