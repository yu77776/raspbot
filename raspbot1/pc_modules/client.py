...
class PCClientWS:
    ...
    def load_model(self):
        ...
        if dev == 'cpu':
            ...
            try:
                ...
                logger.info('cuDNN disabled for compatibility, CUDA still enabled')
            except Exception as e:
                logger.warning('failed to disable cuDNN: %s', e)
    ...
    async def _session(self):
        logger.info('connecting %s ...', self.uri)
                ...
                try:
                    ...
                    if cfg.IMU_DEBUG:
                        ...
                        if now - self._last_env_debug_ts >= cfg.IMU_DEBUG_INTERVAL:
                            ...
                            if imu:
                                logger.debug(
                                    'yaw=%s roll=%s pitch=%s healthy=%s calibrated=%s',
                    ...
                    if now - self._last_env_parse_error_ts >= 5.0:
                        ...
                        logger.warning('parse error: %s: %s bytes=%s', type(exc).__name__, exc, len(message) - 1)
                continue
    ...
    async def _send_loop(self, ws: WebSocketClientProtocol):
        ...
        while True:
            ...
            if payload != last_payload or now >= keepalive_deadline:
                await ws.send(payload)
                last_payload = payload
                keepalive_deadline = now + max(0.1, float(cfg.COMMAND_KEEPALIVE_SEC))
